...
│   ├── cpu.c / cpu.h          ← 6502 CPU emulation
...
- [ ] **M1** — Unity test framework wired up; `make test` runs and passes a hello-world test
- [ ] **M2** — `LDA`, `STA`, `LDX`, `STX`, `LDY`, `STY` (immediate + zero page) passing tests
- [ ] **M3** — All branch instructions correct (all 8 branch opcodes + cycle penalty for page cross)
- [ ] **M4** — All 151 official opcodes passing; nestest.log diff is clean
- [ ] **M5** — iNES ROM parser correct; Mapper 0 PRG ROM reads working
- [ ] **M6** — PPU registers read/write with correct side effects; NMI fires at correct scanline
- [ ] **M7** — Background rendering correct; Donkey Kong title screen renders
- [ ] **M8** — Sprite rendering correct; sprites visible and positioned correctly
- [ ] **M9** — SDL2 window opens; framebuffer blits at 60fps
- [ ] **M10** — Controller input working; Super Mario Bros. playable
- [ ] **M11** — MMC1 (Mapper 1) implemented; The Legend of Zelda boots
- [ ] **M12** — UxROM (Mapper 2) implemented; Contra playable
- [ ] **M13** — APU pulse and triangle channels; basic music audible
- [ ] **M14** — APU noise + DMC; audio complete
- [ ] **M15** — CPU step debugger and memory inspector working
