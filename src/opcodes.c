...
// DEY implied (0x88) -  Decrements the Y register by 1.
...
// JMP indirect (0x6C) - Jump to the address stored at the 16-bit pointer specified in the operand.
...
// AND absolute Y (0x39) - Takes the low byte and a high byte to make a 16-bit address, adds the Y register as an offset (accounts for page cross), and ANDs the value at that address with the accumulator.
...
// AND indirect X (0x21) - Takes the next byte from a zero page address, adds the X register as an offset (wrapping within zero page) that holds a pointer to the value to be ANDed  with the accumulator.
...
// ORA immediate (0x09) - Takes the next byte and does an OR operation on the value stored in the accumulator.
...
// ORA absolute Y (0x19) - Takes the low byte and a high byte to make a 16-bit address, adds the Y register as an offset (accounts for page cross), and ORs the value at that address with the accumulator.
...
// ORA indirect X (0x01) - Takes the next byte from a zero page address, adds the X register as an offset (wrapping within zero page) that holds a pointer to the value to be ORed  with the accumulator.
int op_ora_indirect_x(CPU *cpu, uint8_t *mem)
{
    uint8_t operand = mem[cpu->PC++];
    uint8_t pointer = (uint8_t)(operand + cpu->X);
    uint8_t lo = mem[pointer];
...
int op_eor_absolute_y(CPU *cpu, uint8_t *mem)
{
    uint8_t lo = mem[cpu->PC++];
    uint8_t hi = mem[cpu->PC++];
    uint16_t address = (uint16_t)(hi << 8) | lo;
